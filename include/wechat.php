...
class wechatCallbackapiTest
{
        ...
        if (!empty($postStr)){
            ...
            switch ($msgType) {
                ...
                case "voice":
                    ...
                    $baiduObj = new baiduCallbackapiTest();
                    $reco_baidu = $baiduObj->get_reco("tmp/".$mediaId.".".$format);
                    $time = time();
                    $textTpl = "<xml>
							<ToUserName><![CDATA[%s]]></ToUserName>
							<FromUserName><![CDATA[%s]]></FromUserName>
    ...
    private function get_token() {
        $tokenObj = file_get_contents("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".WECHAT_APP_KEY."&secret=".WECHAT_APP_SECRET);
